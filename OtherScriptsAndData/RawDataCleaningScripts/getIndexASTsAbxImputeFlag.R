getIndexASTsABXsImputeFlag <- function(year=2023L, index_culture_time_filter=42L, end_time_frame=4L) {
   ...
   df <- df %>%
      ...
      distinct()
   df <- rbind(
      df %>% select(-ABX_LABEL) %>% rename(TRT_LABEL = ABXC_LABEL) %>% summarise(NUM_DAYS_TRT = max(NUM_DAYS_TRT), .by=c(PERSON_ID, ORDER_DAY, ORDER_DATE, TRT_LABEL)),
      df %>% select(-ABXC_LABEL) %>% rename(TRT_LABEL = ABX_LABEL) %>% summarise(NUM_DAYS_TRT = max(NUM_DAYS_TRT), .by=c(PERSON_ID, ORDER_DAY, ORDER_DATE, TRT_LABEL))
   )
   df <- df %>%
      #mutate(X = 1L) %>%
      distinct() %>%
   ...
   w <- which(is.na(df$EmpDisc)) # 3,242
   if (length(w) > 0L) {
