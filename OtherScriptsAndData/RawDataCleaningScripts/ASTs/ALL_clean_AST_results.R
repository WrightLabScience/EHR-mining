...
astrDF <- tbl(conn, in_schema('AMB_ETL', 'LAB_MICRO_ALL_VW')) %>% collect()
...
astrDF <- astrDF %>%
   ...
   mutate(
      ...
      STATUS = case_when(
         grepl('SENSITIVE|NEGATIVE', SUSCEPTIBILITY_NAME)              | grepl('Sensitive',              SENSITIVITY_VALUE) ~ 0L,
         grepl('RESISTANT|POSITIVE|INTERMEDIATE', SUSCEPTIBILITY_NAME) | grepl('Resistant|Intermediate', SENSITIVITY_VALUE) ~ 1L
      )
   ) %>%
