plotKP <- function(df, addstatus=FALSE, strat='TRT', cohort='', trt=c('', ''), ymin=0.5, censor_time=30, cex=1, col_vec=c('VAN'='#0000FF', 'iDAP'='#ef5675', 'sDAP'='#ffa600', 'DAP'='#FF0000'), conf_int=TRUE) {
   ...
   if (addstatus) {
      df <- df %>%
         mutate(
            status = ifelse(is.na(time) | time > censor_time, 0, 1),
            time = ifelse(is.na(time) | time > censor_time, censor_time, time)
         )
   }
   
   sample_sizes <- table(df[[strat]])
   fit <- survfit(Surv(time, status) ~ get(strat), data = df)
   d <- split(seq_along(summary(fit)$strata), summary(fit)$strata)
   names(d) <- gsub('get\\(strat\\)=', '', names(d))
   s <- summary(fit)
   xvals <- s$time
   yvals <- s$surv
   ste <- s$std.err
   ...
   legend('bottomleft', text.col=col_vec, pch=NA, x.intersp=0, bty='n', cex=cex,
          ...
          legend=paste0(names(col_vec), ' (n = ', prettyNum(sample_sizes[names(col_vec)], big.mark=','), ')'))
}
