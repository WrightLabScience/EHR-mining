...
w <- which(data$ENC_MISSING_ENCOUNTER_DATA) # 2864
if (length(w) > 0L)
   data <- data[-w,]
data <- data %>% select(-ENC_MISSING_ENCOUNTER_DATA)

w <- which(is.na(data$EmpDisc)) # 1385
if (length(w) > 0L)
   data <- data[-w,]
...
write.csv(data, '~/Desktop/EHR/EHR work/RdataFiles/bacteremia_full_dataset_2017_2023.csv', quote=F, row.names=F, col.names=T)
