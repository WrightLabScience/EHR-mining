...
t <- empDF %>% 
   ...
   filter(total > 200) %>%
   arrange(desc(total))
knitr::kable(t, digits=1, format='pipe')

#    FACILITY     n   disc no_abx  conc    nt
# 1 UPMCPUH   5702 13.0    18.2   63.3  5.44 - Presbyterian
# 2 UPMCSHY   4890 14.6    15.2   65.7  4.58 - Shadyside
# 3 UPMCALT   3296  9.31   11.7   77.5  1.40 - Altoona
# 4 UPMCHAM   3279 10.5    15.9   70.7  2.93 - Hamot        (NWPA + WNY)
# 5 UPMCPAS   3279 12.4    11.6   72.6  3.51 - Passavant    (which one? prob mccandless)
# 6 UPMCMER   2592  9.18   11.8   76.3  2.74 - Mercy
# 7 UPMCSMH   2121 11.8    15.6   68.3  4.24 - St. Margaret
# 8 UPMCEAS   2076 11.6    11.0   73.6  3.85 - East
# 9 UPMCWIL   1664  8.59   10.2   76.3  4.99 - Williamsport
# 10 UPMCJAM   1076 16.6    10.3   70.9  2.14 - Jameson
# 11 UPMCMCK   1049 11.3    11.8   72.8  4.00 - McKeesport
# 12 UPMCMWH    820 11.7    12.8   70.2  5.24 - Magee
# 13 UPMCNOR    702 13.0    12.0   72.8  2.28 - Northwest
# 14 UPMCCHA    632  7.12   12.5   77.7  2.69 - Chatauqua     (NY)
# 15 UPMCHZN    496 10.7    11.3   75.4  2.62 - Horizon       (which one?)
# 16 UPMCBED    357 14.0     7.00  73.9  5.04 - Bedford
# 17 UPMCSOL    229 10.5    12.7   69.4  7.42 - 
# 18 CHP        121  0.826  88.4   10.7  0    - Children's
# 19 UPMCLOC     89  4.49   12.4   78.7  4.49 - 
# 20 UPMCMUN     70 10       7.14  77.1  5.71 - 
# 21 UPMCSUN      6 16.7     0     50   33.3  - 
...
makeFlagPlot <- function(e, main='', leg=FALSE, d) {
   ...
   b <- barplot(h, horiz=TRUE, names.arg=rep('', ncol(h)), legend=leg, xpd=NA, xaxt='n', font.main=4,
                col = gray.colors(n=4, start=0.2, end=0.92), xlim=c(0, 40), space=c(rep(0.1, n), 0.4, 0.1),
...
par(mar=c(2,7,3,4), mgp=c(1.5, 0.5, 0), oma=c(2,0,2,0))
bugs <- c('Enterococcus faecium' = 'VANCOMYCIN', 
          'Staphylococcus aureus' = 'OXACILLIN',
          'Escherichia coli' = 'ESBL', 
          'Klebsiella pneumoniae' = 'ESBL',
...
plot(NA, xlim=c(1,12), ylim=c(1, max(t)), log='y',
     , ylab='Odds ratio', xlab='Number of resistance phenotypes', main='Resistance increases likelihood of > 24 hour delay to concordant therapy')
