...
for (treatment in c('treatment', 'resistance')) {
   ...
   for (cohort in seq_along(cohort_info)) {
      ...
      for (o in seq_along(outcome_vars)) {
         ...
         for (estimand in c('ATE', 'ATT', 'ATC')) {
            ...
            add_covs <- add_covs[add_covs %in% names(data)]
   ...
   save(ATE_estimates, file=sprintf('CohortInfo/%s/ATE_estimates_list.Rdata', treatment))
