...
rm(site_info, code2cat, code2cat_fxn, code2fac, code2fac_fxn, code2mixed, code2mixed_fxn, fac2mixed, fac2mixed_fxn, mixed2cat, mixed2cat_fxn)
...
for (treatment in treatment_vars) {
   ...
   load(file = sprintf('CohortInfo/%s/cohort_info.Rdata', treatment))
   
   # Create plotting directories.
   dir.create(sprintf('Plots/%s/', treatment), showWarnings = FALSE)
   sapply(X = c('/002b_BalanceDiagnostics', '/002b_ClinicalDifferences'),
          FUN = function(dir_name) dir.create(sprintf('Plots/%s/%s', treatment, dir_name), showWarnings = FALSE))
   ...
   for (cohort in seq_along(cohort_info)) {
      ...
      load(file = sprintf('CohortInfo/%s/balancing_weights_info/%s.Rdata', treatment, cohort_name))
      ...
      # Load raw data.
      load(file = sprintf('~/Desktop/EHR/EHR work/RdataFiles/BuildCohortDatasets/CleanedData/%s/%s.Rdata', treatment, cohort_name))
      if (treatment == 'resistance') {
         df$treatment[df$treatment == 1] <- 'R'
         df$treatment[df$treatment == 0] <- 'S'
      }
      ...
      tryCatch(
         {
            # Setup plot space.
            pdf(file = sprintf('Plots/%s/002b_BalanceDiagnostics/%s.pdf', treatment, cohort_name), width=10, height=5.5)
      ...
      tryCatch(
         {
            pdf(file = sprintf('Plots/%s/002b_ClinicalDifferences/%s.pdf', treatment, cohort_name),
            ...
            # Treatment by facility.
            {
               ...
               } else if (treatment == 'resistance') {
                  cat_vec <- df$ENC_admit_facility
                  cat_vec <- fac2cat_fxn(cat_vec)
               }
            ...
            # Continuous variable distributions.
            {
               ...
               if (treatment == 'resistance') {
                  cont_vars[cont_vars == 'ABX_numEmpDoses'] <- 'ABX_numEmpAbxDoses'
                  w <- which(cont_vars == 'ENC_vasopressors')
                  cont_vars <- cont_vars[-w]
                  mains <- mains[-w]
                  xlabs <- xlabs[-w]
                  xmins <- xmins[-w]
                  xmaxs <- xmaxs[-w]
            ...
            # Final plot stuff.
            if (treatment == 'resistance') {
               names(col_vec)[names(col_vec) == '1'] <- 'R'
               names(col_vec)[names(col_vec) == '0'] <- 'S'
            }
            ...
            title_name <- clean_bug_name
            if (treatment == 'resistance')
                title_name <- paste0(title_name, ' - ', abbr[cohort_info[[cohort]]$abx])
            mtext(text=title_name, font=4, outer=TRUE, at=0.5, side=3, line=1, cex=0.7)
            rm(xshift1, xshift2)
