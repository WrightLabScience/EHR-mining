...
for (treatment in c('treatment', 'resistance')) {

   # Load cohort_info list.
   load(file = sprintf('CohortInfo/%s/cohort_info.Rdata', treatment))
   
   load(file = sprintf('CohortInfo/%s/ATE_estimates_list.Rdata', treatment)) # ATE estimates list
   dir.create(sprintf('Plots/%s/003_ATEestimates/', treatment), showWarnings = FALSE)
   ...
   for (cohort in seq_along(ATE_estimates)) {
      ...
      # Load balancing weights info info for extra stats (ess, col_vec).
      ...
      bug_name <- gsub('_resistance', '', cohort_name)
      
      if (treatment == 'treatment' & bug_name == 'S. aureus') 
         bug_name <- paste0(bug_name, ' (MSSA)')
      
      
      tryCatch(
         {
            pdf(file = sprintf('Plots/%s/003_ATEestimates/%s.pdf', treatment, bug_name), height=7.5, width=5.5)
