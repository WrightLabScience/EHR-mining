...
for (treatment in 'resistance') { # treatment_vars) {
   ...
   dir.create(sprintf('CohortInfo/%s/balancing_weights_info/', treatment), showWarnings = FALSE)
   ...
   for (cohort in seq_along(cohort_info)) {
      ...
      if (treatment == 'resistance') {
         df <- df %>% 
            mutate(
               across(
                  .cols = c(ENC_admit_facility, ENC_next_facility),
                  .fns = ~ fac2mixed_fxn(as.character(.))
               )
            )
         
         cat_cols <- names(df)[!sapply(df, function(x) is.numeric(x) | is.integer(x) | is.logical(x))]
         df <- model.matrix(~ . - 1, data = df[cat_cols]) |>
            cbind(df[!names(df) %in% cat_cols])
      }
      ...
      # Save it
      save(balance_list, file = sprintf('CohortInfo/%s/balancing_weights_info/%s.Rdata', treatment, cohort_name))
      cat('\n')
      
      rm(balance_list, cbps.w_list, data, data_mod, df, eb.w_list, ps.gbm, r, sb.w_list, stats_list, sum_balance_mat, w_ATC_energy, 
         w_ATE_energy, w_ATT_energy, weights_list, X, alert, cohort_name, prop_scores, remove_vars, Treatment, v1, v2, var1, var2, z,
         col_vec, ess, cat_cols, i, Rdata_file_path)
   }
}
